...
export function addPages( enabledTools ) {
	...
	if ( enabledTools.hasOwnProperty( "update_acls" ) ) {
		const updateACLs = {
			...
			enabled: () => {
				// Nothing to update?
				if (
					!get( counts ).hasOwnProperty( "zeroed" ) ||
					get( counts ).zeroed < 1 ||
	...
	if ( enabledTools.hasOwnProperty( "copy_buckets" ) ) {
		const copyBuckets = {
			...
			enabled: () => {
				return get( counts ).zeroed > 0 && get( current_settings ).hasOwnProperty( "bucket" ) && copyBuckets.bucket !== get( current_settings ).bucket;
			},
			isNextRoute: ( data ) => {
				if (
					!get( licence ).hasOwnProperty( "is_valid" ) ||
		...
		const uploader = {
			...
			isNextRoute: ( data ) => {
				...
				if (
					...
					!get( licence ).is_valid ||
					!get( current_settings ).hasOwnProperty( "bucket" ) ||
					!get( counts ).hasOwnProperty( "not_zeroed" ) ||
					get( counts ).not_zeroed < 1
				) {
					return false;
				}
